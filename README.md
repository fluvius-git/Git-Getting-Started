...
- all git info sits in the '.git' folder created by 'git init'
<br>
<br>
...
*Remark:* 'git add .' will add all files from the current directory.
...
Now we will edit, thus modify, the README.md file, who was already staged ('add') and see the result if we look at the status again.
 (see below: A became AM)
...
Sometimes you don't want certain files, eg. artefacts of your building proces, to be part of your repository.
To exclude certain file or file types, you get define them in '.gitignore'.
...
```
*Remark*: Iyou don't we to be prompted every time for your login/pswd, you can cache it for a few minutes
```
